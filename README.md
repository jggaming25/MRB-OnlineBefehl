...
**Zugriff live sperren (z. B. außerhalb von Shift/Training):** Im selben Dokument `settings/public` das Feld `accessLocked` (boolean) anlegen. Bei `true` sehen **alle Konten (Tf, Fdl und HR)** sofort – auch in bereits offenen Tabs – nur noch einen weißen Bildschirm ohne Seitenleiste/Dashboard mit der Meldung „Aktuell wird der Zugriff für FDLs und TFs verweigert, da wir nicht innerhalb einer Shift oder eines Trainings sind! ...“ und einem Abmelden-Link. Zum Entsperren muss `accessLocked` wieder auf `false` gesetzt (oder das Feld/Dokument gelöscht) werden – das geht nur über die Firebase-Konsole, da auch HR ausgesperrt ist.
